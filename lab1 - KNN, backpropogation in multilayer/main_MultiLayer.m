...
DTrain = DBins{1};
LTrain = LBins{1};
XTest  = combineBins(XBins, [2:100]);
DTest  = combineBins(DBins, [2:100]);
LTest  = combineBins(LBins, [2:100]);
...
learningRate  = 0.0075; % Change this, your learning rate
W0 = 0; % Initialize your weight matrix W (hidden, samma in och ut)

a = -1; %Random number [-1,1] 
b = 1;

W0 = ((b-a).*rand(width(XTrain),numHidden)+a)/10;  %rader, columner
%Features x classification
V0 = 0; % Initialize your weight matrix V, output (dim hidden layer, output dim)
V0 = randn(numHidden+1,width(DTrain))/10;
