...
for n = 1:numIterations
  ...
  grad_v = HTrain'*(2/NTrain)*(YTrain-DTrain); % 8X1000  * 1000X2
 % Gradient for the output layer
   Vout2 =Vout(1:length(Vout)-1,:); %
   HTrain2=HTrain(:,1:width(HTrain)-1);
   1-HTrain2.^2; %1000x7 
grad_w =(2/NTrain)*XTrain'*((YTrain-DTrain)*Vout2'.*(1-HTrain2.^2));  %1000x7

                     
 % grad_w =(2/NTrain)*(Vout2*(YTrain-DTrain)').*(1-HTrain2.^2);
  % grad_w=0;
   %XTrain'*(2/NTrain)*HTrain2'*(YTrain-DTrain); % And the input layer

  
    %grad_w= 2/NTrain*(XTrain*Wout-DTrain); 
